...
export default function MeetingDetail({ meeting, onBack, onDeleted }: {
  ...
  const fileIcon = (name: string) => {
    if (name.match(/\.(jpg|jpeg|png|gif|webp|svg)$/i)) return '🖼';
    if (name.match(/\.pdf$/i)) return '◇';
    if (name.match(/\.(doc|docx)$/i)) return '◇';
    if (name.match(/\.(xls|xlsx)$/i)) return '◇';
    if (name.match(/\.(zip|rar|7z)$/i)) return '◇';
  ...
  return (
    <div className="max-w-4xl mx-auto p-4 md:p-8 pb-16">
      ...
      >
        {/* Entry Header — Research Archive style */}
            ...
            <Section icon="◆" title={`Action Items (${doneCount}/${items.length})`}>
              ...
              <div className="flex items-center gap-2 mt-3 pt-3 border-t border-journey-border/10">
                <input
                  ...
                  className="flex-1 px-2.5 py-1.5 text-xs font-mono bg-transparent border border-journey-border/20 rounded focus:outline-none focus:border-journey-primary/40 text-journey-text placeholder:text-journey-muted/30"
                />
                <motion.button
                  type="button" whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} onClick={addNewItem}
                  className="px-2.5 py-1.5 text-[10px] font-mono border border-journey-primary/20 text-journey-primary rounded hover:bg-journey-primary/10 transition-colors"
                >
                  + Add
                </motion.button>
              </div>
